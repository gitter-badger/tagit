class Post < ActiveRecord::Base
  ...
  def tags_for_user(user) # Select ([post tags] for [this user]) + ([post tags] for [author + followed users] except those this user already has)
    post_tags
      .select('"tags".name, "post_tags".post_id, "post_tags".tag_id, ("post_tags".user_id = ' + user.id.to_s + ') AS belongs_to_user')
      ...
      .order('"tags".name ASC')
      .group('"tags".name, "post_tags".post_id, "post_tags".tag_id, belongs_to_user')
  end
